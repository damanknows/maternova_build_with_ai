...
export const useEmergencyAlerts = () => {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to fetch emergency alerts'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const addAlert = async (alert: Omit<EmergencyAlert, 'id' | 'created_at' | 'updated_at' | 'status'>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to send emergency alert'));
      return null;
    }
  };
  ...
  const updateAlert = async (id: string, updates: Partial<EmergencyAlert>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to update alert'));
      return null;
    }
  };
  ...
  const deleteAlert = async (id: string) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to delete alert'));
    }
  };
