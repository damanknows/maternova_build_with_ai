...
export const useGovernmentFunding = () => {
  ...
  const fetchFunding = async () => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to fetch government funding'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const addFunding = async (fund: Omit<GovernmentFunding, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to add funding scheme'));
      return null;
    }
  };
  ...
  const updateFunding = async (id: string, updates: Partial<GovernmentFunding>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to update funding scheme'));
      return null;
    }
  };
  ...
  const deleteFunding = async (id: string) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to delete funding scheme'));
    }
  };
