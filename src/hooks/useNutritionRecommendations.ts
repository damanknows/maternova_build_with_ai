...
export const useNutritionRecommendations = () => {
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to fetch nutrition recommendations'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const addRecommendation = async (recommendation: Omit<NutritionRecommendation, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to add nutrition recommendation'));
      return null;
    }
  };
  ...
  const updateRecommendation = async (id: string, updates: Partial<NutritionRecommendation>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to update nutrition recommendation'));
      return null;
    }
  };
  ...
  const deleteRecommendation = async (id: string) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to delete recommendation'));
    }
  };
