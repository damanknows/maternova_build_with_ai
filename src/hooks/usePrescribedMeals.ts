...
export const usePrescribedMeals = () => {
  ...
  const fetchMeals = async () => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to fetch prescribed meals'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const addMeal = async (meal: Omit<PrescribedMeal, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to prescribe meal'));
      return null;
    }
  };
  ...
  const updateMeal = async (id: string, updates: Partial<PrescribedMeal>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to update prescribed meal'));
      return null;
    }
  };
  ...
  const deleteMeal = async (id: string) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to delete prescribed meal'));
    }
  };
