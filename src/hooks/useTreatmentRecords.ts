...
export const useTreatmentRecords = () => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to fetch treatment records'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const addRecord = async (record: Omit<TreatmentRecord, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to add treatment record'));
      return null;
    }
  };
  ...
  const updateRecord = async (id: string, updates: Partial<TreatmentRecord>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to update treatment record'));
      return null;
    }
  };
  ...
  const deleteRecord = async (id: string) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to delete treatment record'));
    }
  };
