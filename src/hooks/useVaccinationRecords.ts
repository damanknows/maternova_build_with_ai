...
export const useVaccinationRecords = () => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to fetch vaccination records'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const addRecord = async (record: Omit<VaccinationRecord, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to add vaccination record'));
      return null;
    }
  };
  ...
  const updateRecord = async (id: string, updates: Partial<VaccinationRecord>) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to update vaccination record'));
      return null;
    }
  };
  ...
  const deleteRecord = async (id: string) => {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Failed to delete vaccination record'));
    }
  };
